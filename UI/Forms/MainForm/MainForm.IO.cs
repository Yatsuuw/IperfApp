...
public partial class MainForm
{
	...
	private async Task ImportConfiguration()
	{
		...
		bool needsMigration = ConfigService.NeedsMigration(content);
		...
		string confirmMsg = needsMigration
			? "Ce fichier est dans un ancien format et sera automatiquement mis à jour.\n\nLa configuration actuelle sera remplacée. Continuer ?"
			: "La configuration actuelle sera remplacée. Continuer ?";

		if (MessageBox.Show(this, confirmMsg, "Confirmer l'importation",
		...
		string successMsg = needsMigration
			? "Configuration importée et mise à jour vers le format actuel."
			: "Configuration importée avec succès.";

		MessageBox.Show(this, successMsg, "Import réussi", MessageBoxButtons.OK, MessageBoxIcon.Information);
	}
